...
| **4 Routing** | `upsert_note` into `Team/` and `Wiki/`, then `curl` **into each backend directly** | the path picks the backend. CouchDB shows a LiveSync *entry* document plus its *leaf* chunk; Algolia shows `note:` + `chunk:` records. `read_file` returns both byte-identical, and `list_children` at the root shows synthesized `Team/`, `Wiki/` beside the physical `Projects/` with no marker distinguishing them |
