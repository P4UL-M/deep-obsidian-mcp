...
fn note_uri(note_path: &str) -> String {
    format!("obsidian://note?path={}", urlencoding::encode(note_path))
}

fn heading_uri(note_path: &str, slug: &str) -> String {
    format!(
...
fn block_uri(note_path: &str, id: &str) -> String {
    format!(
...
pub async fn list_resources(state: &AppState) -> Result<ResourceListResult, String> {
    let snapshot = state.runtime.refresh("resources/list").await?;
    let mut resources = vec![ResourceDefinition {
        uri: "obsidian://vault/info".to_string(),
        name: "vault-overview".to_string(),
        title: Some("Vault Overview".to_string()),
        description: Some("Basic metadata about the configured vault and local search index.".to_string()),
        mime_type: "application/json".to_string(),
    }];
    resources.extend(snapshot.index.file_snapshots.iter().map(|entry| ResourceDefinition {
        uri: note_uri(&entry.path),
    ...
    ResourceTemplateListResult {
        resource_templates: vec![
            ...
            ResourceTemplateDefinition {
                ...
                description: Some("Read the section corresponding to a heading slug within a note.".to_string()),
                mime_type: "text/markdown".to_string(),
            },
            ResourceTemplateDefinition {
                ...
                description: Some("Read a block identified by an Obsidian block id inside a note.".to_string()),
                mime_type: "text/markdown".to_string(),
            },
        ],
    }
...
pub async fn read_resource(state: &AppState, uri: &str) -> Result<ResourceReadResult, String> {
    if uri == "obsidian://vault/info" {
        let snapshot = state.runtime.refresh("resources/read:vault-info").await?;
    ...
    if uri.starts_with("obsidian://note") {
        let path = params.get("path").ok_or_else(|| "missing note path".to_string())?;
    ...
    if uri.starts_with("obsidian://heading") {
        let path = params.get("path").ok_or_else(|| "missing note path".to_string())?;
        let slug = params.get("slug").ok_or_else(|| "missing heading slug".to_string())?;
        let text = read_text(&state.config.vault_path, path).map_err(|error| error.to_string())?;
    ...
    if uri.starts_with("obsidian://block") {
        let path = params.get("path").ok_or_else(|| "missing note path".to_string())?;
        let id = params.get("id").ok_or_else(|| "missing block id".to_string())?;
        let text = read_text(&state.config.vault_path, path).map_err(|error| error.to_string())?;
