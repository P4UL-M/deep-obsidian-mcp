//! Builds note + chunk records from a note's raw content, using the exact same
//! chunk tiling and title/link extraction as the local indexer so boundaries
//! and hashes agree between the exporter and consumers.
...
/// Frontmatter fields the shared records carry as facets, plus the `share`
/// opt-out flag. Parsed from a minimal YAML subset (string scalars, inline
/// `[a, b]` lists, and `- item` block lists) — enough for the vault's
/// conventions without a YAML dependency.
...
pub struct FrontmatterFields {
    ...
    pub tags: Vec<String>,
    /// `share: false` excludes a note from export even inside an included prefix.
...
pub fn parse_frontmatter_fields(content: &str) -> FrontmatterFields {
    ...
    for line in lines {
        ...
        pending_list_key = if value.is_empty() { Some(key.clone()) } else { None };
        match key.as_str() {
...
/// Resolves a raw wiki-link target against the known file set: exact path,
/// path + `.md`, then unique basename-stem match. Unresolvable targets keep
/// their raw form (visible as explicitly unresolved, per design §7.4).
pub fn resolve_link(raw: &str, known_files: &[String]) -> String {
    ...
    // Unique stem match.
    let stem = with_md.rsplit('/').next().unwrap_or(&with_md).to_lowercase();
...
/// Builds the full record set for one version of one note. `known_files` is
/// the file list links are resolved against (exporter: the local vault list).
pub fn build_note_records(
    ...
    let headings: Vec<String> = index_core::extract_heading_sections(content)
        ...
        .collect();
    let content_hash = crate::tools::content_hash(content.as_bytes());
...
mod tests {
    ...
    fn build_note_records_round_trips_chunk_line_ranges() {
        ...
        // Reassembling the sorted chunks must reproduce the source exactly
        // (section chunks tile without overlap).
        ...
        assert_eq!(built.note.content_hash, crate::tools::content_hash(NOTE.as_bytes()));
    }
}
